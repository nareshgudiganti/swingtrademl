...
def walk_forward(
    ...
    for k in range(1, n_folds + 1):
        ...
        # Same preference as chronological_split: the exact per-row
        # label_end_ts when the frame carries it, embargo_days otherwise —
        # so a real training call's folds are purged exactly as its final
        # train/test split is, not by a coarser approximation.
        train_df = (
            before[before["label_end_ts"] < test_start]
            if "label_end_ts" in before.columns
            else apply_embargo(before, embargo_days)
        )
...
def train_model(
    ...
    # embargo_days is passed for a dataset that predates label_end_ts; a real
    # training run's dataset carries it, so the split purges exactly — see
    # chronological_split.
