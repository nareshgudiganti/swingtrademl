...
def test_all_declared_features_are_produced(ohlcv):
...
def test_features_warm_up_and_then_have_no_nans(ohlcv):
    result = build_features(ohlcv)
...
def test_features_do_not_use_future_data(ohlcv):
    ...
    truncated = build_features(ohlcv.iloc[:350])
...
def test_label_is_forward_looking_and_drops_unknown_tail(ohlcv):
    horizon = 5
    labelled = build_label(build_features(ohlcv), horizon_days=horizon, target_return=0.02)
...
def test_latest_feature_row_returns_none_when_not_warmed_up(ohlcv):
    assert latest_feature_row(ohlcv.iloc[:50]) is None

    row = latest_feature_row(ohlcv)
